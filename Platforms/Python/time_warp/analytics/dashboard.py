...
class AnalyticsEngine:
    ...
    def compute_performance_stats(
        ...
        success_count = sum(1 for e in self.collector.get_events(hours=hours)
                          if e.event_type == EventType.CODE_RUN and e.data.get('success', False))
        error_count = sum(1 for e in self.collector.get_events(hours=hours)
    ...
    def get_user_metrics(self, user_id: str, hours: int = 24) -> UserMetrics:
        ...
        execution_times = [e.data.get('execution_time_ms', 0)
                          for e in events if e.event_type == EventType.CODE_RUN]
        ...
        favorite_language = max(languages_used, key=languages_used.get) if languages_used else ""
        ...
        return UserMetrics(
            ...
            last_active=max([e.timestamp for e in events]) if events else datetime.utcnow()
        )
    ...
    def get_system_health(self) -> SystemHealth:
        ...
        return SystemHealth(
            ...
            status="healthy" if error_rate < 5 else "degraded" if error_rate < 15 else "critical"
        )
...
class AlertingSystem:
    ...
    def check_and_alert(self) -> None:
        ...
        # Check latency
        perf_stats = self.analytics.compute_performance_stats(MetricType.LATENCY, hours=1)
        if perf_stats.p99_latency_ms > self.thresholds['p99_latency_ms']:
            self._create_alert(
                'error',
                'High Latency Detected',
                f'P99 latency is {perf_stats.p99_latency_ms:.0f}ms (threshold: {self.thresholds["p99_latency_ms"]}ms)'
            )
        ...
        if health.error_rate_percent > self.thresholds['error_rate_percent']:
            self._create_alert(
                'warning',
                'High Error Rate',
                f'Error rate is {health.error_rate_percent:.1f}% (threshold: {self.thresholds["error_rate_percent"]}%)'
            )
...
class DashboardService:
    ...
    def get_dashboard_data(self) -> Dict:
        ...
        return {
            'performance': self.analytics.compute_performance_stats(MetricType.EXECUTION),
            'health': self.analytics.get_system_health(),
            'active_alerts': self.alerting.get_active_alerts(),
            'metrics_summary': {
                'total_executions': len(self.collector.get_events(event_type=EventType.CODE_RUN)),
                'total_errors': len(self.collector.get_events(event_type=EventType.ERROR)),
                'avg_latency_ms': self.analytics.compute_performance_stats(MetricType.EXECUTION).avg_latency_ms
            }
        }
