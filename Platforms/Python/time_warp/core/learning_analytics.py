...
class LearningAnalytics:
    ...
    def record_program(self,
        ...
        self._trigger_callbacks('program_recorded',
    ...
    def get_progress_metrics(self) -> Dict:
        ...
        mastered = sum(1 for m in self.concept_mastery.values()
                      if m.get_mastery_level() == 'advanced')
        
        return {
            'programs': len(self.programs),
            'successful_rate': successes / len(self.programs),
            'avg_execution_time': sum(p.execution_time for p in self.programs) / len(self.programs),
    ...
    def get_learning_path(self) -> List[str]:
        ...
        # Suggested order: variables → operators → I/O → conditions → loops → functions → arrays
        suggested_order = [
...
"""
        ...
        for concept, info in concept_summary.items():
            ...
            bar = '█' * int(info['confidence'] * 10) + '░' * (10 - int(info['confidence'] * 10))
            report += f"\n{concept:20} [{bar}] {info['confidence']*100:3.0f}% ({level})"
...
"""
        ...
        if error_analysis['most_common']:
            ...
            for i, error in enumerate(error_analysis['most_common'], 1):
                report += f"  {i}. {error['message']} ({error['category']}) - {error['count']} times\n"
...
class ClassroomAnalytics:
    ...
    def get_class_summary(self) -> Dict:
        ...
        avg_success = sum(s.get_progress_metrics()['successful_rate'] 
                         for s in self.students.values()) / len(self.students)
