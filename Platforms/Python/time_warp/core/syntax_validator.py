...
class SyntaxValidator:
    ...
    def _validate_basic(self, code: str) -> List[SyntaxIssue]:
            ...
            if parts and parts[0].isdigit():
                ...
                if line_numbers and line_num <= line_numbers[-1]:
                    issues.append(SyntaxIssue(
                        line=i,
                        column=1,
                        message=f"Line numbers must be in ascending order",
        ...
        open_structures = {'IF': 'THEN', 'FOR': 'NEXT', 'WHILE': 'WEND', 'DO': 'LOOP'}
        ...
        for i, line in enumerate(lines, 1):
            ...
            for close_word in open_structures.values():
                if close_word in stripped:
                    keyword = [k for k, v in open_structures.items() if v == close_word][0]
                    open_count[keyword] -= 1
                    if open_count[keyword] < 0:
                        issues.append(SyntaxIssue(
                            line=i,
                            column=1,
                            message=f"Unexpected {close_word} without matching {keyword}",
                            severity=SeverityLevel.ERROR,
                            code="E002"
                        ))
        ...
        for keyword, count in open_count.items():
            if count > 0:
                issues.append(SyntaxIssue(
                    line=len(lines),
                    column=1,
                    message=f"Unclosed {keyword} structure (missing {open_structures[keyword]})",
                    severity=SeverityLevel.ERROR,
                    code="E003"
                ))
