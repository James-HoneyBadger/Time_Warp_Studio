...
class CommunityMarketplace:
    ...
    def search_items(
        ...
        if query:
            ...
            results = [
                ...
                or any(query_lower in tag for tag in item.tags)
            ]
        ...
        if sort_by == "rating":
            ...
            results.sort(
                key=lambda x: self.stats.get(
                    x.id, ItemStats(x.id, 0, 0, 0, 0, 0, 0, 0, "")
                ).views,
                reverse=True,
            )
    ...
    def remove_from_favorites(self, item_id: str, user_id: str) -> bool:
        ...
        if item_id in self.user_favorites[user_id]:
            ...
            if item_id in self.stats:
                self.stats[item_id].favorites = max(
                    0, self.stats[item_id].favorites - 1
                )
    ...
    def get_author_earnings(self, author_id: str) -> float:
        ...
        for item in self.items.values():
            if item.author_id == author_id and item.price > 0:
                downloads = self.stats.get(
                    item.id, ItemStats(item.id, 0, 0, 0, 0, 0, 0, 0, "")
                ).downloads
