...
class PlayerStats:
    ...
    def record_solve(self, problem_id: str, time_seconds: int, 
                     errors: int = 0, language: str = "BASIC"):
...
class MultiplayerLeaderboard:
    ...
    def register_player(self, username: str) -> PlayerStats:
        ...
        self._trigger_callbacks(self._on_player_joined, {
            "username": username,
            "timestamp": datetime.now().isoformat()
        })
        return player
    
    def record_solve(self, username: str, problem_id: str, 
                     time_seconds: int, errors: int = 0,
                     language: str = "BASIC", is_correct: bool = True):
        ...
        if is_correct:
            ...
            self._trigger_callbacks(self._on_points_earned, {
                "username": username,
                "points": total_points,
    ...
    def create_challenge(self, name: str, description: str, 
                        ...
                        time_limit_minutes: int, max_players: int,
                        code_template: str, test_cases: List[Dict]) -> Challenge:
    ...
    def submit_challenge_solution(self, challenge_id: str, username: str,
                                 code: str, time_seconds: int,
                                 errors: int, test_passed: int,
                                 test_total: int) -> Dict:
        ...
        if username not in self.challenge_results[challenge_id]["participants"]:
            return {"success": False, "message": "Not registered for challenge"}
        ...
        self.challenge_results[challenge_id]["participants"][username]["result"] = result
    ...
    def get_leaderboard(self, leaderboard_type: str = "global", 
        ...
        for ach_id, achievement in self.achievements.items():
            ...
            if achievement.type == AchievementType.SPEED:
                if player.problems_solved >= criteria.get("problems_solved", 1):
                    ...
                    self._trigger_callbacks(self._on_achievement_unlocked, {
                        "username": player.username,
