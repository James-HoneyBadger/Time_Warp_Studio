    ...
    for line in lines:
        ...
        if line_upper.startswith("PRINT "):
            ...
            if content.startswith('"') and content.endswith('"'):
                # String literal
                c_code_lines.append(f'    printf("{content[1:-1]}\\n");')
            else:
        ...
        # Translate LET statements
            ...
            if "=" in line:
                ...
                c_code_lines.append(f"    vars[{var_index}] = {c_expr};")

        # Translate INPUT statements
        elif line_upper.startswith("INPUT "):
            var_name = line[6:].strip().upper()
            var_index = _var_to_index(var_name)
            c_code_lines.append('    printf("? ");')
            c_code_lines.append(f'    scanf("%lf", &vars[{var_index}]);')
