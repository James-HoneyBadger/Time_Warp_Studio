...
class SecurityAuditor:
    ...
    def check_dependencies(self, requirements: List[Tuple[str, str]]) -> List[DependencyVulnerability]:
        """Check dependencies for known vulnerabilities"""
        vulnerabilities: List[DependencyVulnerability] = []
        
        # Known vulnerable packages (simplified database)
        known_vulns = {
            'requests': [{'version': '<2.25.0', 'id': 'CVE-2020-28493', 'title': 'Vulnerable to XXE'}],
            'django': [{'version': '<3.0', 'id': 'CVE-2019-12781', 'title': 'SQL injection vulnerability'}],
            'pillow': [{'version': '<8.0', 'id': 'CVE-2021-23437', 'title': 'Buffer overflow in Image.crop()'}]
        }
...
class CodeAuditService:
    ...
    def full_audit(
        ...
        maintainability_score, maint_issues = self.maintainability_analyzer.analyze(code)
        ...
        critical_count = sum(1 for s in security_issues if s.severity == SecurityLevel.CRITICAL)
    ...
    def generate_audit_report(self, report: CodeQualityReport) -> str:
        ...
        for issue in report.security_issues:
            lines.append(f"[{issue.severity.value.upper()}] {issue.title} ({issue.file}:{issue.line})")
