...
BASIC_PATTERNS: Dict[str, re.Pattern] = {
    ...
    'CIRCLE': re.compile(
        r'CIRCLE\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)\s*,\s*(\d+)',
        re.IGNORECASE
    ),
    'DRAW': re.compile(r'DRAW\s+"([^"]+)"', re.IGNORECASE),
    'PAINT': re.compile(
        r'PAINT\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)(?:\s*,\s*(\d+))?',
        re.IGNORECASE
    ),
    ...
    'MID': re.compile(r'MID\s*\(\s*(.+?)\s*,\s*(\d+)(?:\s*,\s*(\d+))?\s*\)', re.IGNORECASE),
...
LOGO_PATTERNS: Dict[str, re.Pattern] = {
    ...
    'SETPOS': re.compile(r'SETPOS|SETPOSITION\s+\[\s*(.+?)\s+(.+?)\s*\]', re.IGNORECASE),
...
PILOT_PATTERNS: Dict[str, re.Pattern] = {
    ...
    # Matching
    'PATTERN_MATCH': re.compile(r'M:\s*\$([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+)', re.IGNORECASE),
