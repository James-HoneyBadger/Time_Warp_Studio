...
def execute_javascript(interpreter: "Interpreter", source: str, turtle: "TurtleState") -> str:
...
def _js_str(v) -> str:
    if v is None or isinstance(v, _JSNull): return "null"
...
class JSArray(list):
    ...
    def push(self, *items): self.extend(items); return len(self)
    def pop_js(self): return super().pop() if self else _undefined
    def shift(self): return self.pop(0) if self else _undefined
    def unshift(self, *items):
        for x in reversed(items): self.insert(0, x)
        return len(self)
    def splice(self, start, delete_count=None, *items):
        ...
        for i, item in enumerate(items): self.insert(start+i, item)
        return removed
    def slice_js(self, start=None, end=None): return JSArray(self[start:end])
    def concat_js(self, *others):
        r = JSArray(self)
        for o in others: r.extend(o) if isinstance(o, list) else r.append(o)
        return r
    concat = concat_js
    def join(self, sep=","):   return sep.join(_js_str(x) for x in self)
    def indexOf(self, item, start=0):
        try: return self.index(item, start)
    ...
    def lastIndexOf(self, item, end=None):
        ...
        return -1
    def includes(self, item, start=0): return item in self[start:]
    def find(self, fn):
        for x in self:
            if fn(x): return x
        return _undefined
    ...
    def findLastIndex(self, fn):
        ...
        return -1
    def filter(self, fn):  return JSArray(x for x in self if fn(x))
    def map(self, fn):     return JSArray(fn(x) for x in self)
    ...
    def reduceRight(self, fn, init=_undefined):
        ...
        return acc
    def forEach(self, fn):
        for i, x in enumerate(self): fn(x, i, self)
    def some(self, fn): return any(fn(x) for x in self)
    def every(self, fn): return all(fn(x) for x in self)
    def sort(self, fn=None):
        if fn is None: list.sort(self, key=lambda x: _js_str(x))
        else:
            import functools; list.sort(self, key=functools.cmp_to_key(fn))
        return self
    ...
    def flat(self, depth=1):
        ...
        def _f(a, d):
            for x in a:
                (r.extend if (isinstance(x, list) and d > 0) else r.append)(x) if not isinstance(x, list) else _f(x, d-1) if d > 0 else r.append(x)
        _f(self, depth); return r
    def flatMap(self, fn):
        r = JSArray()
        for i, x in enumerate(self):
            m = fn(x, i, self)
    ...
    def fill(self, v, start=0, end=None):
        ...
        for i in range(start, end):
            if 0 <= i < len(self): self[i] = v
        return self
    def copyWithin(self, target, start=0, end=None):
        if end is None: end = len(self)
        src = self[start:end]; t = target if target >= 0 else max(0, len(self)+target)
        for i, v in enumerate(src):
            if 0 <= t+i < len(self): self[t+i] = v
        return self
    def at(self, index):
        try: return self[index]
        except IndexError: return _undefined
    def keys(self):    return JSArray(range(len(self)))
    def values(self):  return JSArray(self)
    def entries(self): return JSArray(JSArray([i, v]) for i, v in enumerate(self))
    def with_js(self, index, v):
        r = JSArray(self); r[index] = v; return r
    def toReversed(self): r = JSArray(self); r.reverse(); return r
    def toSorted(self, fn=None): r = JSArray(self); r.sort(fn); return r
    def toSpliced(self, start, del_count, *items):
        r = JSArray(self); r.splice(start, del_count, *items); return r
    def __repr__(self): return "[" + ", ".join(map(_js_str, self)) + "]"

    @staticmethod
    def isArray(v): return isinstance(v, (list, JSArray))
    @staticmethod
    def from_(it, fn=None):
        a = JSArray(it)
        return JSArray(fn(x, i) for i, x in enumerate(a)) if fn else a
    @staticmethod
    def of(*args): return JSArray(args)
...
class _NumberMeta(type):
    ...
    isInteger = staticmethod(lambda x: isinstance(x, int) or (isinstance(x, float) and x == int(x) and _math.isfinite(x)))
    isSafeInteger = staticmethod(lambda x: isinstance(x, int) or (isinstance(x, float) and x == int(x) and abs(x) <= 9007199254740991))
...
class JSMath:
    ...
    LN2 = _math.log(2); LN10 = _math.log(10)
    LOG2E = _math.log2(_math.e); LOG10E = _math.log10(_math.e)
    SQRT2 = _math.sqrt(2); SQRT1_2 = _math.sqrt(0.5)
    ...
    exp = staticmethod(_math.exp); expm1 = staticmethod(_math.expm1); log1p = staticmethod(_math.log1p)
    sin = staticmethod(_math.sin); cos = staticmethod(_math.cos); tan = staticmethod(_math.tan)
    asin = staticmethod(_math.asin); acos = staticmethod(_math.acos); atan = staticmethod(_math.atan)
    atan2 = staticmethod(_math.atan2)
    sinh = staticmethod(_math.sinh); cosh = staticmethod(_math.cosh); tanh = staticmethod(_math.tanh)
    asinh = staticmethod(_math.asinh); acosh = staticmethod(_math.acosh); atanh = staticmethod(_math.atanh)
    ...
    imul = staticmethod(lambda a, b: ((int(a)*int(b)) & 0xFFFFFFFF) - (((int(a)*int(b)) & 0x80000000) << 1))
    clz32 = staticmethod(lambda x: (32-(int(x) & 0xFFFFFFFF).bit_length()) if x else 32)
...
class JSObject:
    @staticmethod
    def keys(obj): return JSArray(obj.keys()) if isinstance(obj, dict) else JSArray(k for k in vars(obj) if not k.startswith("_"))
    @staticmethod
    def values(obj): return JSArray(obj.values()) if isinstance(obj, dict) else JSArray(vars(obj).values())
    @staticmethod
    def entries(obj):
    ...
    def assign(target, *sources):
        for s in sources:
            if isinstance(s, dict) and isinstance(target, dict): target.update(s)
        return target
    @staticmethod
    def create(proto, props=None): return dict(proto) if isinstance(proto, dict) else {}
    @staticmethod
    def freeze(obj): return obj
    @staticmethod
    def seal(obj): return obj
    @staticmethod
    def is_(a, b): return a is b or (a == b and type(a) is type(b))
    @staticmethod
    def hasOwn(obj, key): return key in obj if isinstance(obj, dict) else hasattr(obj, key)
    @staticmethod
    def getOwnPropertyNames(obj): return JSArray(obj.keys()) if isinstance(obj, dict) else JSArray(dir(obj))
    @staticmethod
    def fromEntries(it): return {k: v for k, v in it}
    @staticmethod
    def defineProperty(obj, key, desc):
        if isinstance(obj, dict): obj[key] = desc.get("value", _undefined)
        return obj
    @staticmethod
    def getPrototypeOf(obj): return None
    @staticmethod
    def setPrototypeOf(obj, proto): return obj
...
class JSJSON:
    ...
    def stringify(obj, replacer=None, indent=None):
        def conv(o):
            if isinstance(o, JSArray): return [conv(x) for x in o]
            if isinstance(o, dict):    return {k: conv(v) for k, v in o.items()}
            if isinstance(o, (_JSNull, _JSUndefined)) or o is None: return None
            if isinstance(o, bool): return o
            return o
        try: return _json.dumps(conv(obj), indent=indent, ensure_ascii=False)
        except: return '""'
    @staticmethod
    def parse(s):
        def restore(o):
            if isinstance(o, list): return JSArray(restore(x) for x in o)
            if isinstance(o, dict): return _JsDict({k: restore(v) for k, v in o.items()})
            return o
...
class JSDate:
    def __init__(self, *args):
        if not args: self._ms = _time.time() * 1000
        elif len(args) == 1:
            if isinstance(args[0], (int, float)): self._ms = float(args[0])
            elif isinstance(args[0], str):
                import datetime as _dt
                try: self._ms = _dt.datetime.fromisoformat(args[0].replace("Z", "+00:00")).timestamp()*1000
                except: self._ms = float("nan")
            else: self._ms = _time.time()*1000
        else:
            import datetime as _dt
            y,mo,d = int(args[0]),int(args[1] if len(args)>1 else 0),int(args[2] if len(args)>2 else 1)
            h,mi,s,ms = int(args[3] if len(args)>3 else 0),int(args[4] if len(args)>4 else 0),int(args[5] if len(args)>5 else 0),int(args[6] if len(args)>6 else 0)
            dt = _dt.datetime(y, mo+1, d, h, mi, s, ms*1000); self._ms = dt.timestamp()*1000
    def _dt(self):
        import datetime; return datetime.datetime.fromtimestamp(self._ms/1000)
    def getTime(self): return int(self._ms)
    def getFullYear(self): return self._dt().year
    def getMonth(self): return self._dt().month-1
    def getDate(self): return self._dt().day
    def getDay(self): return (self._dt().weekday()+1) % 7  # 0=Sun
    ...
    def setFullYear(self, y): d=self._dt().replace(year=int(y)); self._ms=d.timestamp()*1000
    def setMonth(self, m): d=self._dt().replace(month=int(m)+1); self._ms=d.timestamp()*1000
    def setDate(self, n): d=self._dt().replace(day=int(n)); self._ms=d.timestamp()*1000
    def setHours(self, h): d=self._dt().replace(hour=int(h)); self._ms=d.timestamp()*1000
    def setMinutes(self, m): d=self._dt().replace(minute=int(m)); self._ms=d.timestamp()*1000
    def setSeconds(self, s): d=self._dt().replace(second=int(s)); self._ms=d.timestamp()*1000
    def toISOString(self): return self._dt().strftime("%Y-%m-%dT%H:%M:%S.") + f"{self._dt().microsecond//1000:03d}Z"
    def toDateString(self): return self._dt().strftime("%a %b %d %Y")
    def toTimeString(self): return self._dt().strftime("%H:%M:%S GMT+0000")
    def toString(self): return self._dt().strftime("%a %b %d %Y %H:%M:%S GMT+0000")
    def toLocaleDateString(self): return self._dt().strftime("%m/%d/%Y")
    def toLocaleTimeString(self): return self._dt().strftime("%H:%M:%S")
    def toLocaleString(self): return str(self._dt())
    def valueOf(self): return int(self._ms)
    def __repr__(self): return self.toString()
    def __str__(self): return self.toString()
    @staticmethod
    def now(): return int(_time.time()*1000)
    @staticmethod
    def parse(s):
        import datetime as _dt
        try: return int(_dt.datetime.fromisoformat(s.replace("Z","+00:00")).timestamp()*1000)
        except: return float("nan")
    @staticmethod
    def UTC(*args):
        import datetime as _dt
        ...
        h,mi,s = int(args[3] if len(args)>3 else 0),int(args[4] if len(args)>4 else 0),int(args[5] if len(args)>5 else 0)
...
class JSMap:
    def __init__(self, it=None):
        self._d = {}
        if it:
            for k,v in it: self._d[k] = v
    def set(self,k,v): self._d[k]=v; return self
    def get(self,k): return self._d.get(k, _undefined)
    def has(self,k): return k in self._d
    def delete(self,k): removed=k in self._d; self._d.pop(k,None); return removed
    def clear(self): self._d.clear()
    @property
    def size(self): return len(self._d)
    def keys(self): return JSArray(self._d.keys())
    def values(self): return JSArray(self._d.values())
    def entries(self): return JSArray(JSArray([k,v]) for k,v in self._d.items())
    def forEach(self, fn): [fn(v,k,self) for k,v in self._d.items()]
    def __repr__(self): return f"Map({{{', '.join(f'{k!r}: {v!r}' for k,v in self._d.items())}}})"

class JSSet:
    def __init__(self, it=None):
        self._d = {}
        if it:
            for x in it: self._d[x] = True
    def add(self,v): self._d[v]=True; return self
    def has(self,v): return v in self._d
    def delete(self,v): removed=v in self._d; self._d.pop(v,None); return removed
    def clear(self): self._d.clear()
    @property
    def size(self): return len(self._d)
    def keys(self): return JSArray(self._d.keys())
    def values(self): return JSArray(self._d.keys())
    def entries(self): return JSArray(JSArray([k,k]) for k in self._d)
    def forEach(self, fn): [fn(v,v,self) for v in self._d]
    def __repr__(self): return f"Set({{{', '.join(repr(k) for k in self._d)}}})"

class JSWeakMap:
    def __init__(self): self._d = {}
    def set(self,k,v): self._d[id(k)]=(k,v); return self
    def get(self,k): item=self._d.get(id(k)); return item[1] if item else _undefined
    def has(self,k): return id(k) in self._d
    def delete(self,k): self._d.pop(id(k),None)

class JSWeakSet:
    def __init__(self): self._d = set()
    def add(self,v): self._d.add(id(v)); return self
    def has(self,v): return id(v) in self._d
    def delete(self,v): self._d.discard(id(v))
...
class JSRegExp:
    def __init__(self, pattern, flags_str=""):
        self.source = pattern; self._fs = flags_str
        fl = 0
        if "i" in flags_str: fl |= re.IGNORECASE
        if "m" in flags_str: fl |= re.MULTILINE
        if "s" in flags_str: fl |= re.DOTALL
        try: self._re = re.compile(pattern, fl)
        except: self._re = re.compile(re.escape(pattern), fl)
        self.global_flag = "g" in flags_str; self.lastIndex = 0
    def test(self,s): return bool(self._re.search(str(s)))
    def exec(self,s):
        m = self._re.search(str(s))
        if not m: return _null
        r = JSArray([m.group(0)]+list(m.groups())); r.index = m.start(); r.input = s; return r
    def __repr__(self): return f"/{self.source}/{self._fs}"
...
class JSSymbol:
    _c = 0
    def __init__(self,desc=""): JSSymbol._c+=1; self._id=JSSymbol._c; self._desc=desc
    def __repr__(self): return f"Symbol({self._desc})"
    def __hash__(self): return self._id
    def __eq__(self,o): return self is o
    @staticmethod
    def for_key(k): return JSSymbol(k)
...
class JSPromise:
    def __init__(self, executor=None):
        self._val=_undefined; self._err=_undefined; self._state="pending"
        if executor:
            def res(v): self._val=v; self._state="fulfilled"
            def rej(e): self._err=e; self._state="rejected"
            try: executor(res, rej)
            except Exception as e: rej(e)
    ...
    def finally_(self,fn=None):
        if fn: fn(); return self
    @staticmethod
    def resolve(v): p=JSPromise(); p._val=v; p._state="fulfilled"; return p
    @staticmethod
    def reject(e): p=JSPromise(); p._err=e; p._state="rejected"; return p
    @staticmethod
    def all(it): return JSPromise.resolve(JSArray(p._val for p in it))
    @staticmethod
    def allSettled(it):
        def mk(p): return {"status":p._state,"value":p._val} if p._state=="fulfilled" else {"status":p._state,"reason":p._err}
        return JSPromise.resolve(JSArray(mk(p) for p in it))
    @staticmethod
    def race(it): return next(iter(it))
    @staticmethod
    def any(it):
        for p in it:
            if p._state=="fulfilled": return JSPromise.resolve(p._val)
...
class JSStringConstructor:
    @staticmethod
    def fromCharCode(*codes): return "".join(chr(int(c)) for c in codes)
    @staticmethod
    def fromCodePoint(*codes): return "".join(chr(int(c)) for c in codes)
    def __call__(self, v=""): return _js_str(v)
...
class JSEnvironment:
    ...
    def _build_globals(self, captured):
        ...
        class _Con:
            @staticmethod
            def log(*a): captured.append(" ".join(_js_str(x) for x in a))
            @staticmethod
            def error(*a): captured.append("❌ " + " ".join(_js_str(x) for x in a))
            @staticmethod
            def warn(*a): captured.append("⚠️  " + " ".join(_js_str(x) for x in a))
            @staticmethod
            def info(*a): captured.append("ℹ️  " + " ".join(_js_str(x) for x in a))
            @staticmethod
            def dir(o): captured.append(repr(o))
            @staticmethod
            def table(o):
                if isinstance(o, list): [captured.append(f"  {i}: {x}") for i,x in enumerate(o)]
                else: captured.append(repr(o))
            assert_ = staticmethod(lambda c,*a: captured.append("Assertion failed: "+" ".join(_js_str(x) for x in a)) if not c else None)
            time = staticmethod(lambda l="": None); timeEnd = staticmethod(lambda l="": None)
            group = staticmethod(lambda *a: None); groupEnd = staticmethod(lambda: None)

        tc = [0]
        def _sto(fn, ms=0, *a): tc[0]+=1; (fn(*a) if callable(fn) else None); return tc[0]
        def _sin(fn, ms=0, *a): tc[0]+=1; return tc[0]
        ...
        g.update({
            ...
            "encodeURIComponent": lambda s: re.sub(r"[^A-Za-z0-9\-_.!~*'()]", lambda m: "%"+"".join(f"{b:02X}" for b in m.group(0).encode()), str(s)),
            "decodeURIComponent": lambda s: re.sub(r"%([0-9A-Fa-f]{2})", lambda m: chr(int(m.group(1),16)), str(s)),
            "encodeURI": lambda s: str(s),
            "decodeURI": lambda s: str(s),
            "alert": lambda msg: captured.append(_js_str(msg)),
            "prompt": lambda msg="": (interp.request_input(str(msg)) if hasattr(interp,"request_input") else ""),
...
def _parse_int(s, base=10):
    try:
        s = str(s).strip()
        if base == 16 or s.startswith(("0x","0X")): s = s[2:] if s.startswith(("0x","0X")) else s; base = 16
        elif base == 8 or s.startswith("0o"): s = s[2:] if s.startswith("0o") else s; base = 8
        elif base == 2 or s.startswith("0b"): s = s[2:] if s.startswith("0b") else s; base = 2
...
def _fix_switch_indent(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        if "__switch__" in stripped and "=" in stripped and not stripped.startswith("if") and not stripped.startswith("elif"):
        ...
        elif switch_indent >= 0 and stripped == "else:" and cur_indent == switch_indent + switch_body_offset:
        ...
        elif switch_indent >= 0 and cur_indent > switch_indent and switch_body_offset > 0:
...
def _try_expand_line(s: str) -> str:
    ...
    if m and not s.startswith("if") and not s.startswith("for") and not s.startswith("while") and not s.startswith("function"):
        body = m.group(2)
        lines_in_body = [b.strip() for b in body.split(";") if b.strip()]
        body_str = "\n  ".join(lines_in_body)
        return f"{m.group(1)} {{\n  {body_str}\n}}"
...
def _strip_inline_comment(s: str) -> str:
    res = []; in_str = False; sc = ""; i = 0
    while i < len(s):
        c = s[i]
        if in_str:
            res.append(c)
            if c == "\\" and i+1 < len(s): res.append(s[i+1]); i += 2; continue
            if c == sc: in_str = False
        else:
            if c in ('"',"'","`"): in_str = True; sc = c; res.append(c)
...
def _translate_line(line: str) -> str:
    ...
    if re.match(r"^//", s): return pfx + "# " + s[2:]
    s = _strip_inline_comment(s)
    s = s.rstrip(";").rstrip()
    if not s: return ""
    if s.strip() in ("{","}"): return ""

    # } while (...) — do-while terminator (must be BEFORE } stripping)
    m = re.match(r"^\}\s*while\s*\((.+)\)\s*;?$", s)
    if m: return pfx + f"if not ({_translate_expr(m.group(1).strip())}): break"
    ...
    # ── for loops ─────────────────────────────────────────────────────────
    m = re.match(r"^for\s*\(\s*(?:var|let|const)?\s*(\w+)\s*=\s*(.+?);\s*.+?;\s*.+?\s*\)\s*\{?$", s)
    if m:
    ...
    if m:
        lhs = m.group(1).strip(); rhs = _translate_expr(m.group(2).strip())
        if lhs.startswith("[") and lhs.endswith("]"): lhs = lhs[1:-1]
        return pfx + f"for {lhs} in {rhs}:"

    m = re.match(r"^for\s*\(\s*(?:var|let|const)?\s*(\w+)\s+in\s+(.+?)\s*\)\s*\{?$", s)
    if m: return pfx + f"for {m.group(1)} in {_translate_expr(m.group(2).strip())}:"

    # while
    m = re.match(r"^while\s*\((.+)\)\s*\{?$", s)
    if m: return pfx + f"while {_translate_expr(m.group(1).strip())}:"
    if re.match(r"^do\s*\{?$", s): return pfx + "while True:"
    m = re.match(r"^\}\s*while\s*\((.+)\)\s*;?$", s)
    if m: return pfx + f"if not ({_translate_expr(m.group(1).strip())}): break"

    # switch/case/default
    m = re.match(r"^switch\s*\((.+)\)\s*\{?$", s)
    if m: return pfx + f"__switch__ = {_translate_expr(m.group(1).strip())}"
    m = re.match(r"^case\s+(.+?)\s*:$", s)
    if m: return pfx + f"if __switch__ == {_translate_expr(m.group(1).strip())}:"
    if s.strip() == "default:": return pfx + "else:"
    if s.strip() == "break": return pfx + "break"
    if s.strip() == "continue": return pfx + "continue"

    # if/else
    m = re.match(r"^if\s*\((.+)\)\s*\{?$", s)
    if m: return pfx + f"if {_translate_expr(m.group(1).strip())}:"
    m = re.match(r"^else\s+if\s*\((.+)\)\s*\{?$", s)
    if m: return pfx + f"elif {_translate_expr(m.group(1).strip())}:"
    if re.match(r"^else\s*\{?$", s): return pfx + "else:"

    # try/catch/finally
    if re.match(r"^try\s*\{?$", s): return pfx + "try:"
    m = re.match(r"^catch\s*(?:\((\w+)\))?\s*\{?$", s)
    if m: return pfx + (f"except Exception as {m.group(1)}:" if m.group(1) else "except Exception:")
    if re.match(r"^finally\s*\{?$", s): return pfx + "finally:"

    # class
    m = re.match(r"^class\s+(\w+)(?:\s+extends\s+(\w+))?\s*\{?$", s)
    if m: return pfx + (f"class {m.group(1)}({m.group(2)}):" if m.group(2) else f"class {m.group(1)}:")
    ...
    if m: return pfx + f"def {m.group(1)}({m.group(2)}):"
    m = re.match(r"^(\w+)\s*=\s*(?:async\s+)?\(([^)]*)\)\s*=>\s*(.+)$", s)
    if m: return pfx + f"def {m.group(1)}({m.group(2)}): return {_translate_expr(m.group(3))}"
    ...
    if m: v=m.group(1).strip(); return pfx + ("return" if not v else f"return {_translate_expr(v)}")
    m = re.match(r"^throw\s+(.*)", s)
    if m: return pfx + f"raise Exception({_translate_expr(m.group(1))})"

    # __nonlocal__ placeholder (generated by closure expansion)
    m = re.match(r"^__nonlocal__\s+(.+)$", s)
    if m: return pfx + f"nonlocal {m.group(1)}"
...
def _translate_expr(expr: str) -> str:
    ...
    expr = re.sub(r"\.charCodeAt\(([^)]+)\)", r"[ord(\1) if isinstance(\1, str) else ord(chr(int(\1)))]", expr)
    expr = re.sub(r"\.slice\(", "._js_slice(", expr) if "JSArray" in expr or ".slice(" not in expr else expr
    ...
    expr = re.sub(r"(\[[^\]]*\])\.(map|filter|reduce|forEach|some|every|find|findIndex|sort|reverse|join|indexOf|includes|concat|slice)\(", r"JSArray(\1).\2(", expr)

    # Convert variable-based array method calls to JSArray: a.map( → JSArray(a).map(
    expr = re.sub(r"\b([A-Za-z_]\w*)\.(map|filter|reduce|forEach|some|every|find|findIndex|sort|reverse|join|indexOf|concat|flat|flatMap)\(", r"JSArray(\1).\2(", expr)
    ...
    if "?" in expr and ":" in expr:
        ...
        for idx, ch in enumerate(expr):
            ...
            elif ch == "?" and depth == 0 and qi == -1: qi = idx
            elif ch == ":" and depth == 0 and ci == -1 and qi != -1: ci = idx
        if qi != -1 and ci != -1 and qi < ci:
            cond = expr[:qi].strip(); tv = expr[qi+1:ci].strip(); fv = expr[ci+1:].strip()
...
def _fix_object_literals(expr: str) -> str:
    ...
    while i < n:
        if expr[i] == "{":
            ...
            if re.search(r"(?:^|,)\s*(\w+)\s*:", inner) and not re.search(r"\bfor\b|\bif\b", inner):
                # Quote unquoted keys
