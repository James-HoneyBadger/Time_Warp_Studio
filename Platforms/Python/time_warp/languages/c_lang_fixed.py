...
_DECL_RE = re.compile(
    r"^\s*(int|long|float|double)\s+([A-Za-z_][A-Za-z0-9_]*)(\s*=\s*[^;]+)?\s*;?\s*$"
)
...
_WHILE_AFTER_DO_RE = re.compile(r"^\s*while\s*\((.*)\)\s*;?\s*$", re.IGNORECASE)
...
def _exec_c_side_effect_expr(interpreter: "Interpreter", expr: str):
    ...
    if not s:
        return
    # Handle i++, ++i, i--, --i
    m = re.match(r"^\s*(\+\+|--)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*(\+\+|--)?\s*$", s)
    if m:
...
def _scanf(interpreter: "Interpreter", arglist: str) -> str:
    ...
    if "%d" in fmt or "%i" in fmt:
        ...
        tcode = "$"
    interpreter.start_input_request("", name + tcode, is_numeric=(tcode != "$"))
    return ""
...
def _declare_variable(interpreter: "Interpreter", t: str, name: str, init: str | None):
...
def execute_c(interpreter: "Interpreter", command: str, _turtle=None) -> str:
    ...
    # Handle closing brace with a trailing while on same line: `} while (cond);`
    ...
    if low == "break":
        for i in range(len(interpreter.c_block_stack) - 1, -1, -1):
            ...
            if fr.get("type") in ("while", "do"):
                interpreter.current_line = fr.get("end", interpreter.current_line)
                return ""
        return ""
    if low == "continue":
        for i in range(len(interpreter.c_block_stack) - 1, -1, -1):
            ...
            if fr.get("type") == "while":
                interpreter.current_line = fr.get("end", interpreter.current_line) - 1
                return ""
        return ""
        ...
        if cond_val:
            if else_end is not None:
                interpreter.c_block_stack.append(
                    {"type": "if", "end": end_idx, "else_skip_to": else_end + 1}
                )
    ...
    if m:
        ...
        start = _first_inside_index(interpreter, header_idx)
        interpreter.c_block_stack.append(
            {"type": "while", "end": end_idx, "start": start, "cond": cond_expr}
        )
        interpreter.current_line = start - 1
        return ""
    ...
    if m:
        ...
        end_idx = _find_block_end(interpreter, header_idx)
        try:
            cond_val = interpreter.evaluate_expression(cond_expr.strip() or "0")
        except Exception:  # noqa: BLE001
            cond_val = 0
        if not cond_val:
            interpreter.current_line = end_idx
