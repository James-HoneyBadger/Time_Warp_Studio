    ...
    def _step(self, expr, env: Env) -> Any:
        ...
        if head == Symbol("cond"):
            for clause in expr[1:]:
                if clause[0] == Symbol("else") or _scheme_truthy(
                    self.eval_expr(clause[0], env)
                ):
        ...
        if head == Symbol("case"):
            ...
            for clause in expr[2:]:
                if clause[0] == Symbol("else") or (
                    isinstance(clause[0], list) and key in clause[0]
                ):
                    return TailCall(["begin"] + clause[1:], env)
            return None
                        ...
                        for rule in rules_:
                            ...
                            if _syntax_match(pattern, use, literals_, bindings):
                                return self.eval_expr(
                                    _syntax_expand(template, bindings), env_
                                )
        ...
        if head == Symbol("define-record-type"):
            ...
            def make_ctor(cn, flds):
                def ctor(*args):
                    return {
                        str(f): args[i] if i < len(args) else None
        ...
        if head == Symbol("guard"):
            ...
            try:
                ...
                for clause in clauses:
                    if clause[0] == Symbol("else") or _scheme_truthy(
                        self.eval_expr(clause[0], handler_env)
                    ):
        ...
        env.update(
            {
                ...
                "car": lambda a: (
                    ...
                    else (_ for _ in ()).throw(SchemeError("car: empty list"))
                ),
                "cdr": lambda a: (
                    a[1:]
                    if isinstance(a, list) and a
                    else (_ for _ in ()).throw(SchemeError("cdr: empty list"))
                ),
                ...
                "assq": lambda k, lst: next(
                    (p for p in lst if isinstance(p, list) and p and p[0] is k), False
                ),
                "member": lambda k, lst: (sub := [x for x in lst if x == k])
                and sub
                or False,
                ...
                "read": lambda: (
                    self.interpreter.request_input("scheme> ")
                    if hasattr(self.interpreter, "request_input")
                    else ""
                ),
                "error": lambda msg, *irritants: (_ for _ in ()).throw(
                    SchemeError(str(msg) + " " + " ".join(map(scheme_str, irritants)))
                ),
                "void": lambda: None,
                "#t": True,
                "#f": False,
                ...
                "inexact": float,
                "exact->inexact": float,
                "inexact->exact": lambda x: (
                    int(x) if isinstance(x, float) and x.is_integer() else x
                ),
                "integer?": lambda x: isinstance(x, (int, float))
                and (isinstance(x, int) or float(x).is_integer()),
                "real?": lambda x: isinstance(x, (int, float))
                and not isinstance(x, bool),
                "rational?": lambda x: isinstance(x, (int, float))
                and not isinstance(x, bool),
                "complex?": lambda x: isinstance(x, (int, float))
                and not isinstance(x, bool),
                "finite?": lambda x: not (
                    isinstance(x, float) and (x != x or abs(x) == float("inf"))
                ),
                ...
                "gcd": lambda *a: (
                    __import__("math").gcd(*[abs(int(x)) for x in a]) if a else 0
                ),
                "lcm": lambda *a: (
                    round(
                        abs(
                            __import__("functools").reduce(
                                lambda x, y: x * y // (__import__("math").gcd(x, y)),
                                [abs(int(x)) for x in a],
                            )
                        )
                    )
                    if a
                ...
                "asin": math.asin,
                "acos": math.acos,
                "atan": lambda a, b=None: (
                    math.atan2(a, b) if b is not None else math.atan(a)
                ),
                "exp": math.exp,
                "log": lambda x, b=None: (
                ...
                "exact-integer-sqrt": lambda x: (
                    int(math.isqrt(int(x))),
                    int(x) - int(math.isqrt(int(x))) ** 2,
                ),
                ...
                "string-for-each": lambda f, *lsts: [f(*args) for args in zip(*lsts)]
                and None,
                ...
                "string->number": lambda s, r=10: (
                    int(str(s), int(r)) if "." not in str(s) else float(str(s))
                ),
                ...
                "iota": lambda count, start=0, step=1: [
                    start + i * step for i in range(int(count))
                ],
                ...
                "assv": lambda k, lst: next(
                    (p for p in lst if isinstance(p, list) and p and p[0] == k), False
                ),
                "memv": lambda k, lst: (sub := [x for x in lst if x == k])
                and sub
                or False,
                ...
                "partition": lambda f, lst: (
                    [x for x in lst if _scheme_truthy(f(x))],
                    [x for x in lst if not _scheme_truthy(f(x))],
                ),
                "count": lambda f, lst: sum(1 for x in lst if _scheme_truthy(f(x))),
                "sort": lambda lst, cmp=None: (
                    sorted(lst, key=lambda x: x)
                    if cmp is None
                    else sorted(lst, key=lambda x: x)
                ),
                "vector-for-each": lambda f, *vecs: [f(*args) for args in zip(*vecs)]
                and None,
                ...
                "vector-fill!": lambda v, fill, i=0, j=None: [
                    v.__setitem__(k, fill) for k in range(int(i), j or len(v))
                ]
                and None,
                "vector-copy!": lambda to, at, frm, start=0, end=None: [
                    to.__setitem__(int(at) + k - int(start), frm[k])
                    for k in range(int(start), end or len(frm))
                ]
                and None,
                # ── Port/IO functions ──────────────────────────────────────────
                ...
                "read-line": lambda *a: (
                    self.interpreter.request_input("")
                    if hasattr(self.interpreter, "request_input")
                    else ""
                ),
                ...
                "call-with-values": lambda producer, consumer: consumer(
                    *(producer() if isinstance(producer(), tuple) else [producer()])
                ),
                # ── Misc ──────────────────────────────────────────────────────
                ...
                "raise-continuable": lambda x: (_ for _ in ()).throw(
                    SchemeError(str(x))
                ),
                ...
                "force": lambda promise: (
                    promise
                    if not isinstance(promise, SchemeProc)
                    else self.eval_expr([promise], env)
                ),
                "make-promise": lambda x: x,
                "promise?": lambda x: True,
                "make-parameter": lambda val, *a: (
                    lambda v: lambda *nv: v if not nv else None
                )(val),
                "exit": lambda *a: None,
                "command-line": lambda: [],
