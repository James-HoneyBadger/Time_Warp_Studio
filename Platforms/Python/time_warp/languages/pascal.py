...
_VAR_RE = re.compile(
    r"^\s*var\s+(.+):\s*(integer|longint|real|string)\s*;?\s*$", re.IGNORECASE
)
_ASSIGN_RE = re.compile(r"^\s*([A-Za-z_][A-Za-z0-9_]*)\s*:=\s*(.+);?\s*$")
_WRITE_RE = re.compile(r"^\s*(writeln|write)\s*\((.*)\)\s*;?\s*$", re.IGNORECASE)
_READLN_RE = re.compile(r"^\s*readln\s*\((.*)\)\s*;?\s*$", re.IGNORECASE)
_IF_THEN_RE = re.compile(r"^\s*if\s+(.+)\s+then\s*;?\s*$", re.IGNORECASE)
_ELSE_RE = re.compile(r"^\s*else\s*;?\s*$", re.IGNORECASE)
_WHILE_DO_RE = re.compile(r"^\s*while\s+(.+)\s+do\s*;?\s*$", re.IGNORECASE)
_REPEAT_RE = re.compile(r"^\s*repeat\s*$", re.IGNORECASE)
_UNTIL_RE = re.compile(r"^\s*until\s+(.+)\s*;?\s*$", re.IGNORECASE)
_FOR_RE = re.compile(
    r"^\s*for\s+([A-Za-z_][A-Za-z0-9_]*)\s*:=\s*(.+)\s+(to|downto)\s+(.+)\s+do\s*$",
    re.IGNORECASE,
)
...
_FUNC_RE = re.compile(
    r"^\s*function\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?:\((.*)\))?\s*(?::\s*(integer|longint|real|string))?\s*;\s*$",
    re.IGNORECASE,
)
...
def _find_begin_forward(interpreter: "Interpreter", start_idx: int) -> int | None:
    j = start_idx + 1
...
def _parse_param_list(params: str | None) -> List[Dict[str, str]]:
    ...
    for g in groups:
        ...
        for nm in np.split(","):
            ...
            if not name:
                continue
            res.append({"name": name, "type": t, "suffix": suf, "byref": byref})
    return res
...
def _handle_function_def(
    interpreter: "Interpreter", name: str, params: List[Dict[str, str]], rtype: str
):
    """Register a function definition, its return type, and skip its body."""
    begin_idx = _find_begin_forward(interpreter, interpreter.current_line)
    if begin_idx is None:
...
def _handle_proc_call(interpreter: "Interpreter", name: str, arg_str: str) -> str:
    """Invoke a procedure/function call with parameter binding and by-ref support."""
    if not hasattr(interpreter, "pascal_procs"):
        return ""
...
def _handle_while_do(interpreter: "Interpreter", cond_expr: str) -> str:
    ...
    if not cond_v:
        ...
        return ""
    interpreter.pascal_block_stack.append(
        {"type": "while", "start": begin_idx + 1, "end": end_idx, "cond": cond_expr}
    )
    interpreter.current_line = begin_idx + 1 - 1
    return ""
...
def _handle_repeat(interpreter: "Interpreter") -> str:
    ...
    interpreter.pascal_block_stack.append(
        {"type": "repeat", "start": interpreter.current_line + 1, "until": j}
    )
    return ""
...
def _handle_until(interpreter: "Interpreter", cond_expr: str) -> str:
    ...
    if cond_v:
        ...
        interpreter.current_line = top.get("start", interpreter.current_line) - 1
    return ""


def _handle_for(
    interpreter: "Interpreter", name: str, expr_start: str, dirw: str, expr_end: str
) -> str:
...
def execute_pascal(interpreter: "Interpreter", command: str, _turtle) -> str:
    ...
    # Handle end of single-line CASE branch by skipping to CASE end on next line
    if getattr(interpreter, "pascal_block_stack", []):
        ...
        if (
            ...
            interpreter.current_line = top.get("case_end", interpreter.current_line)
            interpreter.pascal_block_stack.pop()
            return ""
    ...
    if _is_end(cmd) and getattr(interpreter, "pascal_block_stack", []):
        ...
        if top.get("end") == interpreter.current_line:
            if top.get("type") == "while":
                try:
                    cond_v = interpreter.evaluate_expression(top.get("cond", "0"))
                except Exception:  # noqa: BLE001
                    cond_v = 0
                if cond_v:
                ...
                interpreter.current_line = top.get("skip_else_to") - 1
                interpreter.pascal_block_stack.pop()
    ...
    # Procedure/function return: when reaching the end index of a called proc, return
    if hasattr(interpreter, "pascal_call_stack") and interpreter.pascal_call_stack:
        ...
        if interpreter.current_line == frame.get("end"):
            ...
            # Skip restoring values for by-ref parameters (allow mutation to persist)
            ...
            interpreter.current_line = frame.get("return_to", interpreter.current_line)
            return ""
        ...
        for a in args:
            ...
            if a.strip().startswith('"') or a.strip().startswith("'"):
                ...
                if up + "$" in interpreter.string_variables:
                    ...
                    try:
                        out_parts.append(f"{interpreter.evaluate_expression(a):g}")
                    except Exception:  # noqa: BLE001
                        out_parts.append("0")
        text = "".join(out_parts)
        if fn.lower() == "writeln":
    ...
    if m:
        ...
        if suf is None:
            suf = "#"
        interpreter.start_input_request("", name + suf, is_numeric=(suf != "$"))
        return ""
