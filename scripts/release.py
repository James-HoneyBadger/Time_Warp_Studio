...
def tag_release(version: str, push: bool = False) -> int:
    ...
    # Create tag
    result = subprocess.run(
        ["git", "tag", "-a", tag, "-m", f"Release {version}"], check=False
    )
    ...
    if push:
        ...
        return subprocess.run(["git", "push", "origin", tag], check=False).returncode
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Time Warp IDE release automation")
    ...
    parser.add_argument(
        "--push", action="store_true", help="Push tag to remote (implies --tag)"
    )
