...
def build_rust(args: argparse.Namespace) -> int:
    """Build Rust implementation."""
    info("Building Rust implementation...")

    root = Path(__file__).parent.parent
    rust_dir = root / "platforms" / "rust"

    if not rust_dir.exists():
        error("Rust platform directory not found")
        return 1
    ...
    return run_command(cmd, cwd=rust_dir)
...
def clean(_args: argparse.Namespace) -> int:
    ...
    # Rust artifacts
    rust_target = root / "platforms" / "rust" / "target"
    if rust_target.exists():
        info("Cleaning Rust target directory...")
        run_command(["cargo", "clean"], cwd=root / "platforms" / "rust")
...
def main() -> int:
    ...
    # Build commands
    build_parser = subparsers.add_parser("build", help="Build platform variants")
    build_parser.add_argument(
        "platform", choices=["rust", "win2000", "all"], help="Platform to build"
    )
    build_parser.add_argument(
        "--release", action="store_true", help="Build in release mode (Rust only)"
    )
    ...
    handlers = {
        "build": lambda: (
            build_rust(args)
            if args.platform == "rust"
            else (
                build_win2000(args)
                if args.platform == "win2000"
                else (build_rust(args) or build_win2000(args))
            )
        ),
