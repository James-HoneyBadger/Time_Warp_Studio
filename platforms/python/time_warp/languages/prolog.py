...
def _unify(x: str, y: str, env: Dict[str, str]) -> Optional[Dict[str, str]]:
    # Both variables: do not bind variable to variable name; treat as same placeholder
    if _is_var(x) and _is_var(y):
        ...
        if x in env and y not in env:
            return env  # y remains unbound, shares x's bound value implicitly in later unifications
...
def _solve_goals_cut(
    ...
    if pred == "neq" and len(args) == 2:
        a, b = args
        # Succeeds only if cannot unify given current bindings (no new bindings created)
    ...
    # Regular predicate: resolve all matches then continue; respect cut propagation
    prune_here = False
    for env1 in _solve(kb, pred, args):
        merged = env.copy()
        merged.update(env1)
    ...
    if prune_here:
        ...
        if not out:
            # No successful solutions but cut committed: propagate cut-fail sentinel
            return [({"__CUTFAIL__": "1"}, True)]
    return out
...
def _solve(kb, pred: str, args: Tuple[str, ...]) -> List[Dict[str, str]]:
    ...
    # Built-in predicates for direct queries or rule heads treated as virtual facts
    ...
    for hp, hargs, body_goals in kb["rules"]:
        ...
        if _body_has_cut(body_goals):
            ...
            if not first_envs:
                ...
                continue
            # Cut reached: evaluate post goals; do not allow backtracking over pre
            ...
            # Evaluate body goals sequentially without pre-binding (sequential unification)
