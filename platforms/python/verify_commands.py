...
def test_pilot_commands():
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program("T:Hello World\nE:")
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program("R:This is a comment\nT:Text\nE:")
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program("T:Before end\nE:\nT:After end")
        output = interp.execute(turtle)
...
def test_basic_commands():
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('10 PRINT "Start"\n20 GOTO 40\n30 PRINT "Skip"\n40 PRINT "End"')
        output = interp.execute(turtle)
        assert "Start" in str(output) and "End" in str(output)
...
def test_logo_commands():
    ...
    try:
        ...
        interp.load_program('CLEARSCREEN')
        output = interp.execute(turtle)
        assert len(turtle.lines) == 0
    ...
    try:
        ...
        interp.load_program('SHOWTURTLE')
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('SETXY 50 100')
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('SETHEADING 180')
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('SETPENCOLOR 255 0 0')
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('SETBGCOLOR 0 0 255')
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('SETPENWIDTH 5')
        output = interp.execute(turtle)
    ...
    try:
        interp = Interpreter()
        turtle = TurtleState()
        interp.load_program('REPEAT 4 [ FORWARD 50 RIGHT 90 ]')
        output = interp.execute(turtle)
...
def main():
    ...
    print()
    print("╔" + "=" * 68 + "╗")
    print("║" + " " * 15 + "TIME WARP IDE - COMMAND VERIFICATION" + " " * 16 + "║")
    print("╚" + "=" * 68 + "╝")
    print()
    ...
    print(f"  {'✅' if pilot_ok else '❌'} PILOT Commands: {'PASSED' if pilot_ok else 'FAILED'}")
    print(f"  {'✅' if basic_ok else '❌'} BASIC Commands: {'PASSED' if basic_ok else 'FAILED'}")
    print(f"  {'✅' if logo_ok else '❌'} Logo Commands:  {'PASSED' if logo_ok else 'FAILED'}")
    print()
