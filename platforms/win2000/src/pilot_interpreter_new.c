...
static void set_var(const TCHAR *name, const TCHAR *val)
{
    ...
    {
        if (_tcsicmp(g_vars[i].name, name) == 0)
        {
            idx = i;
            break;
        }
    }
    if (idx < 0 && g_varCount < 128)
    {
        idx = g_varCount++;
    }
    if (idx >= 0)
    {
        _tcsncpy(g_vars[idx].name, name, 31);
        g_vars[idx].name[31] = 0;
        _tcsncpy(g_vars[idx].value, val, 255);
        g_vars[idx].value[255] = 0;
        TCHAR *end;
        g_vars[idx].numValue = _tcstod(val, &end);
...
static double get_var_num(const TCHAR *name)
{
    for (int i = 0; i < g_varCount; i++)
    {
        if (_tcsicmp(g_vars[i].name, name) == 0 && g_vars[i].isNumeric)
...
static BOOL eval_condition(const TCHAR *cond)
{
    ...
    const TCHAR *p = cond;
    int i = 0;
    while (*p && !(_tcschr(TEXT("=<>"), *p)) && i < 63)
    {
        left[i++] = *p++;
    }
    left[i] = 0;
    i = 0;
    while (*p && (_tcschr(TEXT("=<>"), *p)) && i < 3)
    {
        op[i++] = *p++;
    }
    op[i] = 0;
...
BOOL PilotInterpreter_Execute(const TCHAR *code, HWND hwndConsole, HWND hwndCanvas, BOOL debugMode)
{
    ...
    for (int i = 0; i < lineCount && g_running; i++)
    {
        ...
        {
            TCHAR pat[128];
            _tcsncpy(pat, arg, 127);
            pat[127] = 0;
            CharUpperBuff(pat, _tcslen(pat));
            TCHAR inp[256];
            _tcsncpy(inp, g_lastInput, 255);
            inp[255] = 0;
